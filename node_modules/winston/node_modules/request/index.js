	...
	if (self.proxy) {
		...
		if (http.globalAgent && self.uri.protocol === "https:") {
			...
			var tunnelOptions = { proxy: { host: self.proxy.hostname, port: +self.proxy.port, proxyAuth: self.proxy.auth, headers: { Host: self.uri.hostname + ':' +
				(self.uri.port || self.uri.protocol === 'https:' ? 443 : 80) }}, ca: this.ca }
...
Request.prototype._updateProtocol = function () {
	...
	if (protocol === 'https:') {
		...
		if (self.proxy) {
			...
			var tunnelFn = self.proxy.protocol === 'http:'
				? tunnel.httpsOverHttp : tunnel.httpsOverHttps
			var tunnelOptions = { proxy: { host: self.proxy.hostname, port: +self.proxy.port, proxyAuth: self.proxy.auth }, ca: self.ca }
			self.agent = tunnelFn(tunnelOptions)
			return
		}
