...
Test.prototype.assert = function (res, fn) {
	...
	for (var field in fields) {
		...
		if (null == actual) {
			return fn(new Error('expected "' + field + '" header field'));
		}
		for (var i = 0; i < expecteds.length; i++) {
			var fieldExpected = expecteds[i];
			if (fieldExpected == actual) {
				continue;
			}
			if (fieldExpected instanceof RegExp) {
				re = fieldExpected;
			}
			if (re && re.test(actual)) {
				continue;
			}
			if (re) {
				return fn(new Error('expected "' + field + '" matching ' + fieldExpected + ', got "' + actual + '"'));
			}
